...
class RPS(ui.View):
    ...
    def game_embed(self) -> dc.Embed:
        ...
        embed = dc.Embed(color=0x2F3136, description=desc)
    ...
    async def choose(self, inter: dc.Interaction, choice: int) -> None:
        ...
        if not self.played[inter.user]:
            ...
            return await inter.response.send_message(f"You already chose {self.choice(inter.user)}", ephemeral=True)
        ...
        if [i for i in self.played.values()] == [True, True]:
            ...
            async def tie():
                embed = dc.Embed(title="Game ended with a tie!",
                                 description=f"You both chose {self.choice(inter.user)}", color=0x2F3136)
                embed.set_image(url="attachment://red-line.jpg")
                await self.message.edit(content=None, view=None, embed=embed, file=dc.File("bot/assets/red-line.jpg"))
            ...
            async def win(result):
                ...
                embed = dc.Embed(
                    title="Game ended.", description=f"{winner.mention} won the game!", color=0x2F3136)
                embed.set_image(url="attachment://green-line.jpg")
                embed.add_field(name="Score:", value=score)

                await self.message.edit(content=None, view=None, embed=embed, file=dc.File("bot/assets/green-line.jpg"))
